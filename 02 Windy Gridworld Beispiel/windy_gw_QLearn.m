function [Q,episodesPerTimesteps, statesInLastEpisode] = windy_gw_QLearn(alpha,epsilonStart,decayingEpsilon,sideII,sideJJ,s_start,s_end,wind,MAX_N_EPISODES)
...
for episode=1:MAX_N_EPISODES
  ...
  while( 1 ) 
    ...
    if( ~( (nextState(1)==s_end(1)) && (nextState(2)==s_end(2)) ) ) % stp1 is not the terminal state
      Q(stateIndex,action) = Q(stateIndex,action) + alpha*( reward + gamma*max(Q(nextStateIndex,:)) - Q(stateIndex,action) ); 
