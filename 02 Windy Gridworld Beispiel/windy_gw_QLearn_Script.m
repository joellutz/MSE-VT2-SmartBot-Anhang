...
[Q,ets, statesInLastEpisode] = windy_gw_QLearn(alpha,epsilon,decayingEpsilon,sideII,sideJJ,s_start,s_end,wind,MAX_N_EPISODES);
...
fn = sprintf('windy_gw_QLearn_policy_nE_%d',MAX_N_EPISODES);
saveas( gcf, fn, 'png' ); 
...
fn = sprintf('windy_gw_QLearn_state_value_fn_nE_%d',MAX_N_EPISODES);
hold on
...
ylabel('episodes')
xlabel('time steps')
fn = sprintf('windy_gw_QLearn_learning_rate_nE_%d',MAX_N_EPISODES);
saveas( gcf, fn, 'png' ); 
